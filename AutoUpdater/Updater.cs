...
namespace AutoUpdater
{
    public class Updater
    {
        ...
        public Updater(string serverIP, int serverPort)
        {
            ...
            // Donnie 将所有需要版本控制的文件放到执行文件上级的source文件夹中
            this.appDirPath = dir.Parent.FullName + "\\source\\";
            
            // Donnie
            EnsureDirectoryExist(this.appDirPath);
        ...
        private void UdpateThread()
        {
            try
            {
                ...
                {
                    // TODO 此处添加弹窗提示，让用户自行选择是否需要更新
                    DialogResult dr = MessageBox.Show("A new version release was detected", "Note:",MessageBoxButtons.OKCancel, MessageBoxIcon.Question);
                    if (dr == DialogResult.OK) //调用messagebox 弹出是、否 按钮对话选择框
                    {
                        //如果选择“是”，则执行以下代码
                    }
                    else
        ...
        private void GetUpdateInfo(out IList<string> downLoadFileNameList, out IList<FileUnit> removeFileNameList)
        {
            ...
            //检查版本控制的文件夹
            //string pathx = directory.FullName + "\\" ;
            string pathx = directory.FullName + "\\source\\" ;
            ...
            foreach (FileUnit fileUnit in fileList)
            {
                ...
                {
                    if (  fileUnit.FileRelativePath  == path)
                    {
                        isLost = true;
                        break;
                    }
        ...
        void FileReceivingEvents_FileTransDisruptted(ITransferingProject transmittingFileInfo, FileTransDisrupttedType fileTransDisrupttedType)
        {
